...
Plan approved. Proceed with all 4 fixes:

1. Reorder model fallback list — gemini-flash-latest first, older models as trailing fallback

2. Fix the health probe — raise maxOutputTokens so reasoning models can respond, and classify by HTTP status (only 401/403/429/hard failures = unusable) instead of by empty text

3. Add a "no model available" signal to run logs for future visibility

4. Re-run auto-fill manually after the fix and confirm admin history shows free keys usable 2/2 with a non-zero saved count

No secret changes needed, keys are already correct.