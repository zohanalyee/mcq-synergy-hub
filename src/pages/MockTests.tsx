...
import { Clock, Award, BarChart2, BookOpen, ArrowDown, ArrowUp, SlidersHorizontal, Check } from "lucide-react";
...
const generateAllMockTests = () => {
  ...
  const subjectsWithoutTests = subjects.filter(subject => !existingCategories.includes(subject.title));
...
const MockTests = () => {
  ...
  const handleStartTest = (test: any, customSettings?: any) => {
    ...
    const testTopics = selectedTopics[test.id] && selectedTopics[test.id].length > 0 ? selectedTopics[test.id] : test.topics;
      ...
      <div className="container mx-auto px-4 pt-28 pb-16">
        ...
        <div className="mb-8 overflow-x-auto pb-2">
          <div className="flex space-x-2 min-w-max">
            {isLoaded && getCategories().map(category => <Button key={category} variant={filter === category ? "default" : "outline"} onClick={() => setFilter(category)} size="sm" className="capitalize">
