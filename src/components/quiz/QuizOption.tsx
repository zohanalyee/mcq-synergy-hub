...
const QuizOption = ({ letter, text, state, disabled, onClick }: QuizOptionProps) => {
  ...
  return (
    <motion.button
      ...
      className={cn(
        "w-full flex items-center gap-3 sm:gap-4 px-4 sm:px-5 py-3.5 sm:py-4 rounded-xl border-2 text-left transition-colors",
    ...
    >
      <span
        className={cn(
          "shrink-0 inline-flex items-center justify-center h-9 w-9 rounded-lg font-bold text-sm",
      ...
      <span className="flex-1 text-sm sm:text-base font-medium leading-snug">{text}</span>
    </motion.button>
  );
};
