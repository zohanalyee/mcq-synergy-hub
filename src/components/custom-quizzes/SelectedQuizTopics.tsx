...
interface SelectedQuizTopicsProps {
  customSubjects: CustomSubject[];
  selectedSubjectsCount: number;
  setSelectedCategory: (category: string) => void;
}

const SelectedQuizTopics = ({ 
  customSubjects, 
  selectedSubjectsCount,
  setSelectedCategory
}: SelectedQuizTopicsProps) => {
  ...
  const selectedTopicsCount = React.useMemo(() => {
    return customSubjects.reduce((count, subject) => {
      return count + subject.topics.filter(topic => topic.selected).length;
    }, 0);
  }, [customSubjects]);
  ...
  return (
    <div className="mt-4">
      <h3 className="text-sm font-medium mb-1.5">Selected Topics</h3>
      <div className="bg-muted/50 rounded-lg p-4 h-[350px] overflow-y-auto">
...
export default SelectedQuizTopics;
