-- Indexes to speed up normalised-text matching across both question pools
CREATE INDEX IF NOT EXISTS idx_content_items_norm_title
  ON public.content_items (md5(lower(btrim(title))))
  WHERE category = 'mcq';

CREATE INDEX IF NOT EXISTS idx_job_test_questions_norm_question
  ON public.job_test_questions (md5(lower(btrim(question))));

CREATE INDEX IF NOT EXISTS idx_job_test_questions_job_test_id
  ON public.job_test_questions (job_test_id);

-- Read function: unified, filterable, sortable, server-side paged view of BOTH pools
...
-- Headline combined stats for the admin dashboard (library + mock-test pools)
